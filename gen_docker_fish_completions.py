...
class Switch(object):
    ...
    @property
    def pwsh_completion(self):
        complete_arg_spec = ['''%s''' % x for x in self.longs]
        desc = str(self.description).replace("'","''")
        return "                [CompletionResult]::new('--{0}', '{0}', [CompletionResultType]::ParameterName, '{1}')".format(' '.join(complete_arg_spec), desc)
...
class BaseFishGenerator(object):
    ...
    def subcommands(self):
        ...
        for sub in self.docker.subcommands():
            ...
            print('''            "$baseCommand;{command}" {{'''.format(
                binary=self.docker.binary,
                command=sub.command,
                desc=desc))
            ...
            for switch in sub.switches:
                print('''{completion}'''.format(
                    binary=self.docker.binary,
                    command=sub.command,
                    completion=switch.pwsh_completion))
...
class DockerFishGeneratorOrig(BaseFishGenerator):
    header_text = """
